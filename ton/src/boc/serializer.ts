...
import { augment, rollback } from '../utils/bits'
...
const deserializeCell = (remainder: number[], refIndexSize: number): CellData => {
    ...
    const pointer: CellPointer = {
        ...
        refs: Array.from({ length: totalRefs }).map(
            () => bytesToUint(remainder.splice(0, refIndexSize))
        )
    }

    return { pointer, remainder }
}
...
const serializeCell = (cell: Cell, hashmap: Map<string, number>, refIndexSize: number): Bit[] => {
    const representation = [].concat(
        cell.getRefsDescriptor(),
        cell.getBitsDescriptor(),
        cell.getAugmentedBits()
    )
